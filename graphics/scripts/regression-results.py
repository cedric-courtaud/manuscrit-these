...
def plot_regression_results(cli_args):
    ...
    plt.scatter(training.observed, training.predicted, c='lightgray')
    plt.scatter(validation.observed, validation.predicted, c='red')
    plt.plot(training.observed, training.observed)
    plt.xlabel('Predicted overhead (%)', fontsize=16)
    plt.ylabel('Observed overhead (%)', fontsize=16)

    plt.title(title_string(features), fontsize=22)

    mae = validation.abs_error.mean()
    mse = validation.squared_error.mean()

    plt.text(300, -200, f'$MSE={mse:.2f}$        $MAE={mae:.2f}$', fontsize=18, horizontalalignment='center')

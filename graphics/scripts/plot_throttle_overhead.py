...
def annotate_death_zone_slides(ax, alpha=0.25):
    mpl.rcParams['hatch.linewidth'] = 6.0  # previous svg hatch linewidth
    x_min, x_max = ax.get_xlim()
    y_min, y_max = ax.get_ylim()
    ax.text(x_max, 320, 'Execution time $\\times 4$',
            horizontalalignment='right', verticalalignment='center', zorder=3)
    ax.axhline(300, 0, 1, ls=':', c='red', zorder=7)
    ax.axhspan(300, ax.get_ylim()[1],0, 1, color='r', fill=False, alpha=alpha, hatch='//', lw=0, zorder=1)
...
def throttle_overhead_all_log(cli_args):
    ...
    annotate_death_zone_slides(ax, alpha=0.1)
    xmax += 0.25 * xmax
...
def do_plot_bw_overhead_annot(df, bw_col, is_big=False, xlabel=None):
    palette = sns.color_palette(len(df.instance.unique()) * ['gray'])
    ...
    g.map(plt.scatter, bw_col, 'overhead', alpha=1, zorder=3, linewidth=None, s=8, color='gray')
    ...
    wcet = df.sort_values(by=bw_col)
    ax.plot(wcet[bw_col], wcet.overhead.cummax(),zorder=4, color='red')

    # ax.axhline(300, 0.025, 0.99, ls=':', c='red')
    ax.axvline(min_300[bw_col], 0.05, 0.95, ls=':', c='red', zorder=5)
    ax.scatter(min_300[bw_col], min_300.overhead, s=7, c='r', zorder=6)
    annotate_death_zone_slides(ax, alpha=0.05)
